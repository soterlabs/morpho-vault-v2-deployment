...
contract DeployUsdtRiskCapital is DeployHelpers, StdCheats {
    ...
    function run() external returns (DeploymentResult memory result) {
        ...
        string memory vaultName = "sky.money USDT Risk Capital";
        string memory vaultSymbol = "skyMoneyUsdtRiskCapital";
    ...
    ///      1. Deposit 1 USDT via the vault (auto-allocated to market via liquidity adapter) → market supply = 1 USDT
    ///      2. Supply 1 USDT directly to the Morpho market → market supply = 2 USDT
    ///      3. Supply 2.1 stUSDS as collateral to the market
    ///      4. Borrow 1.8 USDT from the market → utilization = 1.8 / 2 = 90%
    function _setupDeadDeposits(VaultV2 vault, IMorpho morpho, MarketParams memory params, address deployer) internal {
        // A. Deposit 1 USDT into the vault (sent to 0xdEaD to make shares unrecoverable).
        ...
        // B. Supply 1 USDT directly to the Morpho market (also to 0xdEaD).
        //    Combined with step A, total market supply = 2 USDT.
        ...
        // D. Borrow 1.8 USDT to reach 90% utilization (1.8 / 2 = 90%).
